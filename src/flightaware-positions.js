...
async function fetchPositionForTail(ident, apiKey) {
  try {
    ...
    if (!active) {
      ...
      // Use hardcoded airport coords first (FlightAware's airport coords have
      // been known to return wrong data for some airports). Fall back to FA's
      // destination.latitude/longitude only if the airport isn't in our table.
      const hardcoded = destCode ? lookupAirport(destCode) : null;
      const destLat = hardcoded?.latitude ?? dest.latitude ?? null;
      const destLon = hardcoded?.longitude ?? dest.longitude ?? null;
      console.log(`[fa-positions] ${ident} last landed at ${destCode || '?'} (lat: ${destLat}, lon: ${destLon}, src: ${hardcoded ? 'hardcoded' : 'flightaware'})`);
      ...
      return {
        ...
        groundedCity: hardcoded?.city || dest.city || null,
      };
    }
