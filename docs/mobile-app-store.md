...
Skyway Ops has bundled Capacitor 8 projects for iOS and Android. The mobile
...
This foundation was ported forward from draft PR #7 onto current `main`. The
package ID remains `com.flyskyway.ops`.

## Requirements

- Node.js 20+ (Capacitor 8; 22+ is preferred)
...
- A deployed production API. Mobile builds default to `https://www.skyway.app`.
  Set `VITE_API_BASE_URL` at native-bundle build time if that host changes.
...
Do not invent or commit placeholder `GoogleService-Info.plist` or
`google-services.json` files.

## Build and run

### Verify the toolchain (any OS)
...
`mobile:sync` runs the Vite production build and copies the bundle into the
iOS and Android projects. `mobile:doctor` confirms Capacitor 8 dependencies and
the Android project. On Linux it still exits non-zero because Xcode is missing;
that is expected. Android SDK compilation and Xcode archive are macOS/Studio
steps and are not claimed from a Linux VM.
...
## How native auth reaches the web app

1. `@capacitor-firebase/authentication` opens the native Microsoft flow.
2. The resulting Firebase ID token is POSTed to `/api/mobile-auth-token` on
   `https://www.skyway.app` (or `VITE_API_BASE_URL`).
...
- API behavior against `https://www.skyway.app` and all role-specific workflows
- Home fleet tracking map / data panel and other post-August ops features
